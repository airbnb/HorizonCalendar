...
final class DayOfWeekPositionTests: XCTestCase {
  ...
  func testDayOfWeekComparable() throws {
    for dayOfWeekPositionRawValue in DayOfWeekPosition.allCases.map({ $0.rawValue }) {
      if dayOfWeekPositionRawValue > 1 {
        let dayOfWeekPosition = try XCTUnwrap(DayOfWeekPosition(rawValue: dayOfWeekPositionRawValue))
  ...
  func testDayOfWeekPositionForDate() throws {
    ...
    let date6 = try XCTUnwrap(gregorianCalendar.date(
