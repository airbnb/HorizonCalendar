...
final class MonthRowTests: XCTestCase {
  ...
  func testRowInMonthForDate() throws {
    let date0 = try XCTUnwrap(gregorianCalendar.date(from: DateComponents(year: 2020, month: 06, day: 02)))
    XCTAssert(
      gregorianCalendar.rowInMonth(for: date0) == 0,
      "Expected June 2, 2020 to be in the first row of the month."
    )

    let date1 = try XCTUnwrap(gregorianCalendar.date(from: DateComponents(year: 2020, month: 02, day: 05)))
    XCTAssert(
      gregorianCalendar.rowInMonth(for: date1) == 1,
      "Expected February 5, 2020 to be in the second row of the month."
    )

    let date2 = try XCTUnwrap(gregorianCalendar.date(from: DateComponents(year: 1500, month: 12, day: 15)))
    XCTAssert(
      gregorianCalendar.rowInMonth(for: date2) == 2,
      "Expected December 15, 1500 to be in the third row of the month."
    )

    let date3 = try XCTUnwrap(gregorianCalendar.date(from: DateComponents(year: 0001, month: 02, day: 21)))
    XCTAssert(
      gregorianCalendar.rowInMonth(for: date3) == 3,
      "Expected February 21, 0001 CE to be in the fourth row of the month."
    )

    let date4 = try XCTUnwrap(gregorianCalendar.date(from: DateComponents(year: 2020, month: 06, day: 30)))
    XCTAssert(
      gregorianCalendar.rowInMonth(for: date4) == 4,
      "Expected June 30, 2020 to be in the fifth row of the month."
    )

    let date5 = try XCTUnwrap(gregorianCalendar.date(from: DateComponents(year: 2020, month: 05, day: 31)))
    XCTAssert(
      gregorianCalendar.rowInMonth(for: date5) == 5,
      "Expected May 31, 2020 to be in the sixth row of the month."
    )

    let date6 = try XCTUnwrap(japaneseCalendar.date(from: DateComponents(era: 236, year: 01, month: 03, day: 01)))
    XCTAssert(
      japaneseCalendar.rowInMonth(for: date6) == 0,
      "Expected March 1, 01 era 236 to be in the first row of the month."
    )

    let date7 = try XCTUnwrap(japaneseCalendar.date(from: DateComponents(era: 236, year: 01, month: 03, day: 31)))
    XCTAssert(
