...
final class DayHelperTests: XCTestCase {
  ...
  func testDayContainingDate() throws {
    ...
    let december0005Date = try XCTUnwrap(gregorianCalendar.date(
  ...
  func testStartDateOfDay() throws {
    let november2020Day = Day(
      month: Month(era: 1, year: 2020, month: 11, isInGregorianCalendar: true),
      day: 17
    )
    let november2020Date = try XCTUnwrap(gregorianCalendar.date(
    ...
    let january0100Date = try XCTUnwrap(gregorianCalendar.date(
    ...
    let june02Date = try XCTUnwrap(japaneseCalendar.date(
