...
public enum MonthsLayout: Hashable {
    ...
    var pinDaysOfWeekToTop: Bool {
        switch self {
        case let .vertical(options): options.pinDaysOfWeekToTop
    ...
    var alwaysShowCompleteBoundaryMonths: Bool {
        switch self {
        case let .vertical(options): options.alwaysShowCompleteBoundaryMonths
        case .horizontal: true
        }
    }
    ...
    var isPaginationEnabled: Bool {
        guard
            case let .horizontal(options) = self,
    ...
    var scrollsToFirstMonthOnStatusBarTap: Bool {
        switch self {
        case let .vertical(options): options.scrollsToFirstMonthOnStatusBarTap
        case .horizontal: false
        }
    }
}
...
public struct HorizontalMonthsLayoutOptions: Hashable {
    ...
    func pageSize(calendarWidth: CGFloat, interMonthSpacing: CGFloat) -> CGFloat {
        guard case let .paginatedScrolling(configuration) = scrollingBehavior else {
            preconditionFailure(
...
public extension HorizontalMonthsLayoutOptions {
    /// The scrolling behavior of the horizontally-scrolling calendar: either paginated-scrolling or free-scrolling.
    enum ScrollingBehavior: Hashable {
...
public extension HorizontalMonthsLayoutOptions {
    ...
    struct PaginationConfiguration: Hashable {
        // MARK: Lifecycle
...
public extension HorizontalMonthsLayoutOptions.PaginationConfiguration {
    // MARK: - HorizontalMonthsLayoutOptions.PaginationConfiguration.RestingPosition

    /// The position at which the calendar will come to a rest when paginating.
    enum RestingPosition: Hashable {
